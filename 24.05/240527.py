...
#풀이
# N개의 점중에서 (가장 큰 x값 - 가장 작은 x값 )x(가장 큰 y값 -가장 작은 y값) 을계산하면 대지의 넓이가 나온다.
...
for _ in range(N):
    ...
    x_list.append(x)
    y_list.append(y)
# 땅의 넓이는 (가장 큰 x값 - 가장 작은 x값 )x(가장 큰 y값 -가장 작은 y값)
grand = (max(x_list)-min(x_list))*(max(y_list)-min(y_list))
print(grand)
...
if a+b+c == 180:
    ...
    print("Error")
# 삼각형의 성질 때문에 a + b + c = 180 이므로 처음부터 삼각형 일때와 아닐때를 나눠 반복작업을 줄였다. 